...
View Source: [contracts/mocks/RolesMock.sol](../contracts/mocks/RolesMock.sol)
...
```js
struct Roles.Role private dummyRole;
...
- [has(address account)](#has)

### add

```js
function add(address account) public undefined
```
...
```js
function remove(address account) public undefined
```
...
```js
function has(address account) public
returns(bool)
```
